  ...
  methods: {
    getPageData() {
      ...
      if (pageComponent && pageComponent.data) {
        ...
        console.error(
          '请在front matter中设置pageComponent和pageComponent.data数据'
        )
      }
    },
    getCatalogueList() {
      const { sidebar } = this.$site.themeConfig
      const { key, parent } = this.$frontmatter.pageComponent.data
      let catalogueList

      if (!parent) {
        catalogueList = sidebar[`/${key}/`]
      }
      if (key && parent) {
        const name = key.split('.').pop()
        const menuData = sidebar[`/${parent}/`]?.find((o) => o.title === name)
        catalogueList = menuData?.children
      }

      if (!catalogueList) {
        console.error(
          '未获取到目录数据，请查看front matter中设置的key是否正确。'
        )
      }

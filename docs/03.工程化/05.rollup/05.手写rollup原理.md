---
title: 手写rollup原理
date: 2022-05-11 16:56:42
permalink: /pages/d5d86e/
categories:
  - 工程化
  - rollup
tags:
...
:::theorem 
rollup的核心卖点就是它强大的tree-shaking能力，Vue3就利用它的这个特点设置特性开关，将框架打包各种版本的boundle包。下面我们通过手写一个简版的rollup理解它的核心工作原理。
:::
...
![868f60df0f5227621151031e261678a9](https://xj-1253772569.file.myqcloud.com/868f60df0f5227621151031e261678a9)
...
![1d821a22ff221e924731a6d8c8a654c4](https://xj-1253772569.file.myqcloud.com/1d821a22ff221e924731a6d8c8a654c4)
...
![d434186f338a42917a8ef3835df3a28f](https://xj-1253772569.file.myqcloud.com/d434186f338a42917a8ef3835df3a28f)

#### 3.2.1 walk.js
:::tip
使用`walk`用深度遍历的方式，对AST语法树的每一个节点进行遍历，在进入语法树前子节点执行`enter`函数，离开语法树节点时执行`leave`函数，用于后面分析抽象语法树的每一个节点，做各种各样的处理。
:::
```js
...
![d39f73349c0580b4bfe6aa106ef0b1ae](https://xj-1253772569.file.myqcloud.com/d39f73349c0580b4bfe6aa106ef0b1ae)
...
scope.js
:::tip
使用`Scope`用于表示抽象语法树节点的作用域内声明了哪些变量，还有它的父作用域是哪个。为后面的treeShaking做准备。
:::
```js
class Scope {
  constructor(options = {}) {
...
lib\bundle.js
:::tip
编译入口类, 读取入口js文件, 递归读取将所有js模块, 然后实例化成module, 把模块里面的代码变成MagicString实例便于后续操作。
:::
```js
let fs = require('fs');
...
:::tip
 创建一个模块, 通过`acorn`将代码字符串解析成抽象语法树, 存放所有导入的模块，所有定义的变量，所有导出的变量等等。
::: 
...
class Module {
    ...
    analyse() {
        analyse(this.ast, this.code, this); // 定义ast语法树节点所对应的源码字符串
    }
    expandAllStatements() {
        let allStatements = [];
...
![4324fd261744db779245477d4cc336a6](https://xj-1253772569.file.myqcloud.com/4324fd261744db779245477d4cc336a6)
...
![d6bcbc174f98e9aa834ea48397efb177](https://xj-1253772569.file.myqcloud.com/d6bcbc174f98e9aa834ea48397efb177)

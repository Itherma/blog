---
title: React 和 React-dom
date: 2022-05-29 11:46:53
permalink: /pages/ddc410/
categories:
  - 框架
  - React
tags:
...
- JSX其实只是一种语法糖，最终会通过[Babel](https://www.babeljs.cn/repl)转译成`React.createElement(type, props, children)`语法
- `React.createElement`会返回一个React元素（虚拟dom）
...
ReactDOM.render(element1, document.getElementById("root")); // react18 以前
```
...
export function mount(vdom, container) {
  let newDOM = createDOM(vdom); // 创建真实dom
...
export function createDOM(vdom) {
  ...
  if (props) {
    ...
    if (typeof props.children == "object" && props.children.type) {
      ...
      reconcileChildren(props.children, dom); // 递归创建子元素
    }
  }
  vdom.dom = dom;
  return dom;
...
+        this.pendingStates = []; // 为后面的批量更新,存储需要更新的状态
+        this.callbacks = []; // 回调集合
+    }
...
+        }
+        this.emitUpdate(); // 发布更新
+    }
+    emitUpdate() {
+        this.updateComponent(); // 更新组件
+    }
+    updateComponent() {
+        let { classInstance, pendingStates } = this; // 获取要更新的状态和实例
+        if (pendingStates.length > 0) { // 如果说有需要更新的状态
+            shouldUpdate(classInstance, this.getState()); // 已经拿到新状态
+        }
+    }
+    getState() { // 获得新的状态
...
+            if (typeof nextState === 'function') { // 如果新状态是函数的话, 执行函数得到新状态
+                nextState = nextState(state);
+            }
+            state = { ...state, ...nextState }; // 合并状态,相同属性,新的覆盖老的
+        });
+        pendingStates.length = 0; // 清空待更新的状态数组
+        return state; // 返回新的状态
+    }
+}
+function shouldUpdate(classInstance, nextState) {
+    classInstance.state = nextState; // 更新类组件的状态
+    classInstance.forceUpdate(); // 组件更新
+}
export class Component {
    static isReactComponent = true; // 增加类组件标识, 用于区分类组件还是函数式组件
    constructor(props) {
        this.props = props;
+        this.state = {};
+        this.updater = new Updater(this); // 每个类组件都有自己的更新器
    }
...
+        let newRenderVdom = this.render(); // 根据新的属性得到新的vdom
+        compareTwoVdom(oldDOM.parentNode, oldRenderVdom, newRenderVdom); // 更新dom并挂载
+        this.oldRenderVdom = newRenderVdom; // 将
+    }
+}
```
...
export function createDOM(vdom) {
  ...
  vdom.dom = dom; // 将真实dom节点绑定到vdom上, 便于后面的查找更新
  return dom;
}
function mountClassComponent(vdom) { // 挂载类组价
  let { type, props } = vdom;
  let classInstance = new type(props);
  let renderVdom = classInstance.render();
+ classInstance.oldRenderVdom = renderVdom;  // 把vdom放在class实例上
  let dom = createDOM(renderVdom);
  return dom;
}
function mountFunctionComponent(vdom) { // 挂载函数式组件
  let { type, props } = vdom;
  let renderVdom = type(props);
+ vdom.oldRenderVdom = renderVdom; // 把函数组件生成的新vdom放在当dom.oldRenderVdom前
  return createDOM(renderVdom);
}
function updateProps(dom, oldProps={}, newProps={}) {
    for (let key in newProps) {
...
+export function findDOM(vdom) { // 根据虚拟dom找到真实的dom节点
+    if (!vdom) return null;
+    if (vdom.dom) { // 有dom属性直接返回dom
+        return vdom.dom;
+    } else {
+         // // 函数组件或类组件的情况下需要递归找到真实的dom
...
+       });
    ...
    // 打印结果: 0,0,1,2；在同一个函数里面的setState都会被合并。声明周期钩子里也是，只要能被react识别到都可以。
    // 
    render() {
        return (
            <div>
...
+    isBatchingUpdate:false, // 是否是批量更新, 如果是true就是批量的异步的, 是false就不是批量的同步的
+    updaters:new Set(), // 
+    batchUpdate(){ // 批量更新
+      updateQueue.isBatchingUpdate = false;
+      for (var updater of updateQueue.updaters){
+          updater.updateComponent(); // 更新组件
+      }
+      updateQueue.updaters.clear(); // 清空队列
+    }
+}
class Updater {
    constructor(classInstance) {
...
+       if(updateQueue.isBatchingUpdate){ // 如果是true就把当前的updater放在队列里 
+           updateQueue.updaters.add(this);
+       }else{
+           this.updateComponent(); // 为false就直接更新
+       }
+   }
    updateComponent() {
        let { classInstance, pendingStates } = this;
...
export function addEvent(dom, eventType, handler) { // dom节点, 事件名, 处理函数
    let store = dom.store||(dom.store ={}) // 取store属性, 没有就添加
    store[eventType] = handler; // 把事件类型和处理函数先保存到真实dom节点上
    if (!document[eventType]) { // 不存在这个事件类型就加上
        document[eventType] = dispatchEvent; // 派发事件
    }
}

// 事件代理到document上, 向上冒泡到document上
function dispatchEvent(event) {
  let { target, type } = event; // 拿到事件源dom和事件类型
  let eventType = `on${type}`; // 补全事件类型
  let syntheticEvent = createSyntheticEvent(event);
  updateQueue.isBatchingUpdate = true; // 开启批量更新标识符
  while (target) {
    let { store } = target; // 获取事件源dom上的store
